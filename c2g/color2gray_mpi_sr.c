...
int main(int argc, char *argv[]) {
	...
	unsigned char *global_gray_image = NULL;
    ...
    global_gray_image = (unsigned char *)malloc(sizeof(unsigned char)*gray_image_size);
    if(gray_image == NULL) {
...
//    MPI_Scatter(gray_image_size, 1, MPI_INT, local_gray_image_size, 1, MPI_INT);
    for (int i=1;i<process_count;i++){
    	MPI_Send(&color_image_size, 1, MPI_INT,i, 0 , MPI_COMM_WORLD);
    	MPI_Send(color_image, color_image_size, MPI_UNSIGNED_CHAR,i, 0 , MPI_COMM_WORLD);
    }
    for (int i=1;i<process_count;i++){
    	MPI_Recv(gray_image, gray_image_size, MPI_UNSIGNED_CHAR, i, 0 , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for (int row = 0; row < height; ++row)
        {
            for (int col = 0; col < width; ++col)
            {
               //If the input image has a transparency channel this will be simply copied to the second channel of the gray image, while the first channel of the gray image will contain the gray pixel values. If the input image has three channels, the output image will have only one channel with the gray data.
                ...
                global_gray_image[greyOffset]+=gray_image[greyOffset];
            }
        }
...
//    MPI_Scatter(color_image, &color_image_size, MPI_CHAR, local_color_image, &color_image_size, MPI_CHAR, 0 , MPI_COMM_WORLD);
   //colorToGrayscale(gray_image, color_image, width, height);
	...
	{
		int color_image_size, gray_image_size,rgbOffset;
		MPI_Recv(&color_image_size, 1, MPI_INT,0, 0 , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		gray_image_size = color_image_size / 3;
		gray_image = (unsigned char *)malloc(sizeof(unsigned char)*gray_image_size);
		color_image = (unsigned char *)malloc(sizeof(unsigned char)*color_image_size);
		//printf("image size: %d\n",color_image_size);
		MPI_Recv(color_image, color_image_size, MPI_UNSIGNED_CHAR, 0, 0 , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		//printf("color pixel: %d\n",color_image[0]);
		int step = gray_image_size % (process_count - 1) ==0 ? gray_image_size / (process_count - 1):gray_image_size / (process_count - 1)+1;
		int start = (my_rank-1) * step;
		int end = (my_rank) * step <=  gray_image_size? (my_rank) * step: gray_image_size;
		unsigned char pixel[DESIRED_CHANNELS];
		for (int greyOffset = start; greyOffset<end; greyOffset++){
            rgbOffset = greyOffset * DESIRED_CHANNELS;
		...
		printf("gray pixel: %d\n",gray_image[0]);
		MPI_Send(gray_image, gray_image_size, MPI_UNSIGNED_CHAR,0, 0 , MPI_COMM_WORLD);
	}
		MPI_Finalize();
	return 0;
}
