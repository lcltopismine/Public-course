...
int comparator(const void *a, const void *b) {
	Rankobj * obj_a = (Rankobj *)a;
	Rankobj * obj_b = (Rankobj *)b;
  if (obj_a->value < obj_b->value) {
    return -1;
  } else if (obj_a->value > obj_b->value) {
    return 1;
  } else {
    return 0;
  }
...
// TODO: change type to double
	  ...
	  if (my_rank==0){
		  Rankobj *Rankorder = malloc(process_count * sizeof(Rankobj));
		  int i;
		  for (i = 0; i < process_count; i++) {
			  Rankorder[i].my_rank = i;
		      memcpy(&(Rankorder[i].value), collect_array + (i * dsize), dsize);
		  }
		  qsort(Rankorder, process_count,
			  sizeof(Rankorder), &comparator);

		  for (int i = 0; i < process_count; i++) {
			rank[Rankorder[i].my_rank] = i+1;
		  }
...
int main(int argc, char * argv[]){
	...
	TMPI_Rank(&number, &rank, MPI_INT,
			MPI_COMM_WORLD);
	printf("rank %d comes from process %d with value: %d\n",rank,my_rank,number);
	MPI_Finalize();
	return 0;
