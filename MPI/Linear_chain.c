...
int main(int argc, char * argv[]){
	...
	if (my_rank==0){
		...
		if (my_rank!=process_count-1){
			printf("Receive %d and send %d from process %d with value %d\n", recv_num, random_num,status.MPI_SOURCE, sum);
			MPI_Send(&sum, 1, MPI_INT,next_rank,0,MPI_COMM_WORLD);
		}
		else
		printf("Terminated at rank %d and generated number at this turn is %d and The total sum is: %d\n",my_rank,random_num,sum);
	}
	MPI_Finalize();
	return 0;
}
