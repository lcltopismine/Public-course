...
int main(int argc, char *argv[]) {
    ...
    size_t color_img_size = width * height * gray_channels * 3;
   ...
   //parallel this function
    int block_size = 256;
    dim3 gridDim((gray_img_size-1)/block_size+1,1,1);
    dim3 blockDim(block_size,1,1);
   //start by transferring array
    unsigned char *local_gray_img, *local_color_img;
    cudaMalloc((void **)&local_gray_image,gray_img_size);
    cudaMalloc((void **)&local_color_image,color_img_size);
    cudaMemcpy(local_color_image, color_image, color_img_size, cudaMemcpyHostToDevice);

   colorToGrayscale<<<gridDim,blockDim>>>(local_gray_image, local_color_image, width, height);
   cudaMemcpy(gray_image, local_gray_image, gray_image_size, cudaMemcpyDeviceToMemory);
...
void colorToGrayscale(unsigned char *gray_img, unsigned char * color_img, int width, int height){
   ...
   //If the input image has a transparency channel this will be simply copied to the second channel of the gray image, while the first channel of the gray image will contain the gray pixel values. If the input image has three channels, the output image will have only one channel with the gray data.
	...
	int greyOffset = blockDim.x * blockIdx.x + threadIdx.x;
	if (greyOffset < width*height){
